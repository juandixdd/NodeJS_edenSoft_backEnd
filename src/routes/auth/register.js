...
router.post("/register", (req, res) => {
  ...
  const registerUser = () => {
    const { correo, contrasena, id_cliente_documento, id_rol, img } = req.body;
    const hash = bcryptjs.hashSync(contrasena, 10);
    const query =
      "INSERT INTO usuario (correo, contrasena, id_cliente_documento, id_rol, img) VALUES (?,?,?,?,?)";
    mySqlConnection.query(
      query,
      [correo, hash, id_cliente_documento, id_rol, img],
      (err, rows, fields) => {
        if (!err) {
          res.json({ 
            status: 200, 
